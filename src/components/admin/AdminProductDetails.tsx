...
export const AdminProductDetails = ({ product }: ProductDetailsProps) => {
  return (
        ...
        <div className="grid grid-cols-1 gap-3">
          {product.VariantProducts.map((variant) => (
            <ProductVariantCard
              key={variant.id}
              variant={variant}
              productId={product.id} onStatusChange={function (productId: number, variantId: number, newStatus: StatusType): Promise<void> {
                throw new Error('Function not implemented.');
              } } onDeleteClick={function (variantId: number, productId: number): void {
                throw new Error('Function not implemented.');
              } }            />
          ))}
        </div>
